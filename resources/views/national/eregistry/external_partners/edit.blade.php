...
{{-- <div class="container mx-auto font-poppins px-8 max-w-5xl mt-1"> {{ Breadcrumbs::render('files.create.withType', $createType) }} </div> --}}
    ...
    <form action="{{ route('registry.external-partners.update', $externalPartner->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white shadow-sm font-poppins rounded-2xl p-6 max-w-2xl mx-auto mt-4 mb-4">
            ...
            <h2 class="text-m font-semibold text-gray-800 mb-6">
                <span class="text-cyan-600">Edit Ministry External Partner</span>
            </h2>
            ...
            <div class="mb-3 relative">
                ...
                <input type="text" 
                        ...
                        value="{{ old('name', $externalPartner->name) }}"
            ...
            <div class="mb-3 relative">
                ...
                <input type="email" name="email" id="email"
                    value="{{ old('email', $externalPartner->email) }}"
                    class="mt-1 w-full rounded-lg border-gray-300 shadow-sm 
                        focus:border-cyan-500 focus:ring-2 focus:ring-cyan-200 
                        text-sm px-3 py-2">
            </div>
            ...
            <div class="mb-3 relative">
                ...
                <input type="tel" name="phone" id="phone"
                    value="{{ old('phone', $externalPartner->phone) }}"
                    class="mt-1 w-full rounded-lg border-gray-300 shadow-sm 
                        focus:border-cyan-500 focus:ring-2 focus:ring-cyan-200 
                        text-sm px-3 py-2">
            </div>
                ...
                <select name="identity_organisation_id" id="identity_organisation_id"
                    ...
                    @foreach($identityOrganisations as $org)
                        <option value="{{ $org->id }}"
                            ...
                            @selected(old('identity_organisation_id', $externalPartner->identity_organisation_id) == $org->id)>
                ...
                <select name="organisation_type_id" id="organisation_type_id"
                    ...
                    @foreach($organisationTypes as $type)
                        <option value="{{ $type->id }}"
                            @selected(old('organisation_type_id', $externalPartner->organisation_type_id) == $type->id)>
                            {{ $type->name }}
                        </option>
                    @endforeach
                </select>
