...
<div class="container bg-white mx-auto px-6 py-10 max-w-4xl mt-4 mb-4 text-gray-700 font-medium rounded-md min-h-screen border border-gray-600">
    ...
    <form action="{{ route('registry.files.update', $file) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
            <div class="text-gray-700 text-sm grid grid-cols-1">
                @if(Auth::user()->hasRole('registry'))
                    <div>
                        ...
                        @php
                            $selectedSourceType = old('source_type');

                            if ($selectedSourceType === null) {
                                if (
                                    $file->source_type === App\Models\National\Eregistry\IdentityOrganisation::class &&
                                    $file->source_id == auth()->user()->ministry->identity_organisation_id
                                ) {
                                    $selectedSourceType = 'own_ministry';
                                } elseif ($file->source_type === App\Models\National\Eregistry\IdentityOrganisation::class) {
                                    $selectedSourceType = 'identity_organisation';
                                } elseif ($file->source_type === App\Models\National\Eregistry\ExternalPartner::class) {
                                    $selectedSourceType = 'external_partner';
                                }
                            }
                        @endphp
                        <select name="source_type" id="source_type"
                            ...
                            <option value="own_ministry"
                                {{ $selectedSourceType === 'own_ministry' ? 'selected' : '' }}>
                                My Ministry
                            ...
                            <option value="identity_organisation"
                                {{ $selectedSourceType === 'identity_organisation' ? 'selected' : '' }}>
                                Other Registered Organisations
                            </option>

                            <option value="external_partner"
                                {{ $selectedSourceType === 'external_partner' ? 'selected' : '' }}>
                                External Partners
                            </option>
                        </select>
                    </div>
                                ...
                                <optgroup label="{{ $type }}">
                                    @foreach($organisations as $organisation)
                                        <option value="{{ $organisation->id }}"
                                            @selected(old('source_id', $file->source_id) == $organisation->id)>
                                            {{ $organisation->name }}
                                        </option>
                                    @endforeach
                                </optgroup>
