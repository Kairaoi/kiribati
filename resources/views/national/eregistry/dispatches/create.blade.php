...
<div class="container mx-auto px-4 py-6 max-w-7xl">
    ...
    <form action="{{ route('registry.movements.store') }}" method="POST" class="space-y-6">
        ...
        <!-- From Organisation -->
        <div>
            <label for="from_organisation_id" class="block text-sm font-medium text-gray-700">From Organisation</label>
            <select name="from_organisation_id" id="from_organisation_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm @error('from_organisation_id') is-invalid @enderror">
                <option value="">Select a Organisation</option>
                @foreach ($organisations as $id => $organisation)
                    <option value="{{ $id }}" {{ old('from_organisation_id') == $id ? 'selected' : '' }}>{{ $organisation }}</option>
                @endforeach
            </select>
            @error('from_organisation_id')
                <span class="text-red-600 text-sm">{{ $message }}</span>
            @enderror
        </div>
        ...
        <!-- To Organisation -->
        <div>
            <label for="to_organisation_id" class="block text-sm font-medium text-gray-700">To Organisation</label>
            <select name="to_organisation_id" id="to_organisation_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm @error('to_organisation_id') is-invalid @enderror">
                <option value="">Select a Organisation</option>
                @foreach ($lists as $id => $organisation)
                    <option value="{{ $id }}" {{ old('to_organisation_id') == $id ? 'selected' : '' }}>{{ $organisation }}</option>
                @endforeach
            </select>
            @error('to_organisation_id')
                <span class="text-red-600 text-sm">{{ $message }}</span>
            @enderror
        </div>
