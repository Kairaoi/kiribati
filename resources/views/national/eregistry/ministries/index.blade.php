...
<div class="container mx-auto px-4 py-6 max-w-7xl">
    <div class="flex justify-between items-center mb-4">
        ...
        <a href="{{ route('registry.organisations.create') }}" class="btn btn-primary transition duration-300 transform hover:scale-105">
            <i class="fas fa-plus mr-2"></i> Add New Organisation
        </a>
    </div>

    <div class="bg-white rounded-lg shadow-lg overflow-hidden">
        <table id="organisationsTable" class="table w-full">
...
$(document).ready(function() {
   ...
   $('#organisationsTable').on('click', '.action-dropdown', function(e) {
       e.stopPropagation();
       const button = $(this);
       ...
       const dropdown = $(` 
           <div class="dropdown-menu" style="display:none;">
               <a class="dropdown-item" href="${route('registry.organisations.show', rowId)}">
                   <i class="fas fa-eye text-cyan-500 mr-2"></i> View
               </a>
               <a class="dropdown-item" href="${route('registry.organisations.edit', rowId)}">
                   <i class="fas fa-edit text-cyan-500 mr-2"></i> Edit
               </a>
   ...
   $(document).on('click', '.delete-action', function(e) {
       ...
       if (confirm('Are you sure you want to delete this organisation?')) {
           $.ajax({
               url : route('registry.organisations.destroy', id),
               type : 'DELETE',
               headers :{
                   'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') 
               },
               success(response) {
                   $('#organisationsTable').DataTable().ajax.reload();
   ...
   function route(name, id) {
      return {
          'registry.organisations.show': "{{ route('registry.organisations.show', ':id') }}".replace(':id', id),
          'registry.organisations.edit': "{{ route('registry.organisations.edit', ':id') }}".replace(':id', id),
          'registry.organisations.destroy': "{{ route('registry.organisations.destroy', ':id') }}".replace(':id', id)
      }[name];
   }
});
</script>
